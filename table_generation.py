...
def main():
    ...
    datasets_df = pd.read_csv("./tables/dataset_classification/run_results.csv")
    ...
    datasets_df = datasets_df.drop(columns=["dataset", "traffic_generator", "pcap", "attack", "error"])
    ...
    new_datasets_df = new_datasets_df.drop(columns=[col for col in new_datasets_df.columns if "tool" in col or col == "error"])
    # Reorder
